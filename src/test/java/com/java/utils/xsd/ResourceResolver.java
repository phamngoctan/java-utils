...
//		System.out.println("This LSResourceResolver implementation assumes that all XSD files have a unique name. "
	...
	public LSInput resolveResource(String type, String namespaceURI, String publicId, String systemId, String baseURI) {
		...
		if (!pathMap.containsKey(currentResourceName)) {
			...
			if (systemId.contains("/")) {
				...
				// The current resource should be at the same path as the baseresource
			...
			try {
				...
				// files has a special character as a first character indicating utf-8 file//

				InputStream is = new ByteArrayInputStream(s1.getBytes());
				return new LSInputImpl(publicId, systemId, is); // same as Input
				// class
			} finally {
		        s.close();
		    }
		...
		// If this resource has already been added, do not add the same resource
		// again. It throws
